...
function roomShowsOnToday(
	...
	const roomEvents = bootstrap.events.filter((e) => e.day === guideDay && e.roomId === room.id);
	return roomEvents.some((ev) => eventOverlapsUtcWindow(ev, nowMs, windowEnd, cache));
}
...
function getZonedParts(date: Date, timeZone: string): { ymd: string; minutes: number } {
...
function wireTagDescriptionPopover() {
	...
	const pop = document.getElementById("tag-desc-popover") as HTMLElement & { showPopover?: () => void };
	if (!shell || !pop) return;
...
function roomsUsedOnDay(events: EnrichedEvent[], day: string, roomOrder: Room[]): string[] {
	const ids = new Set(events.filter((e) => e.day === day).map((e) => e.roomId));
	return roomOrder.map((r) => r.id).filter((id) => ids.has(id));
}

function eventsForRoomDay(events: EnrichedEvent[], day: string, roomId: string): EnrichedEvent[] {
	return events
...
function lastEventEndMinutesOnDay(events: EnrichedEvent[], day: string): number | null {
 ...
 */
function findPrimaryAndFollowing(list: EnrichedEvent[], nowMin: number): {
	...
	if (live) {
		const following = list.find((ev) => ev.startMinutes >= live!.endMinutes) ?? null;
		return { primary: live, primaryIsLive: true, following };
	}
	if (firstUpcoming) {
		const following = list.find((ev) => ev.startMinutes >= firstUpcoming.endMinutes) ?? null;
...
function readSelectedTags(): Set<string> {
	...
	document.querySelectorAll<HTMLButtonElement>(".tag-chip[data-active='true']").forEach((b) => {
...
function readSelectedRooms(): Set<string> {
	const out = new Set<string>();
	document.querySelectorAll<HTMLButtonElement>(".tag-chip[data-room-id][data-active='true']").forEach((b) => {
...
function applyScheduleFilters() {
	...
	const allCards = [...scheduleRoot.querySelectorAll<HTMLElement>("[data-event-card]")];

	scheduleRoot.querySelectorAll<HTMLElement>("[data-room-column]").forEach((col) => {
...
function applyNowHighlights(bootstrap: ClientBootstrap, now: Date) {
	...
	document.querySelectorAll<HTMLElement>("[data-event-card]").forEach((card) => {
		const day = card.dataset.day ?? "";
		const sm = Number(card.dataset.startMin ?? "0");
		const em = Number(card.dataset.endMin ?? "0");
		const on = daySet.has(ymd) && day === ymd && sm <= minutes && minutes < em;
...
function wireChipToggles(rerender: () => void) {
	...
	document.querySelectorAll<HTMLButtonElement>(".tag-chip[data-tag-id]").forEach((b) => {
		b.addEventListener("click", () => toggle(b));
	});
	document.querySelectorAll<HTMLButtonElement>(".tag-chip[data-room-id]").forEach((b) => {
		b.addEventListener("click", () => toggle(b));
	});
}
...
function wireFilterPanel() {
	const panel = document.getElementById("filter-panel");
	const btn = document.getElementById("nav-filters") as HTMLButtonElement | null;
	if (!panel || !btn) return;
...
function wireNav(bootstrap: ClientBootstrap) {
	...
	const filterBtn = document.getElementById("nav-filters") as HTMLButtonElement | null;
...
function roomColumnHeadClass(room: Room): string {
	return room.headerTextColor === "light" ? "room-column__head room-column__head--light" : "room-column__head room-column__head--dark";
}

function buildFavoriteEventCard(ev: EnrichedEvent, room: Room, tags: TagDefinition[]): HTMLElement {
	const article = document.createElement("article");
	article.className = ev.highlight ? "event-card event-card--highlight" : "event-card";
...
function renderFavoritesView(bootstrap: ClientBootstrap) {
	...
	for (const day of bootstrap.days) {
		...
		for (const room of bootstrap.rooms) {
			const list = eventsForRoomDay(bootstrap.events, day, room.id).filter((ev) => favIds.has(ev.id));
...
function renderEventMini(root: HTMLElement, ev: EnrichedEvent, room: Room | undefined, tags: TagDefinition[]) {
	const block = document.createElement("div");
	block.className = ev.highlight ? "event-card event-card--highlight" : "event-card";
	if (ev.highlight && room) block.style.setProperty("--room-accent", room.accent);
...
function renderToday(bootstrap: ClientBootstrap) {
	...
	const used = roomsUsedOnDay(bootstrap.events, guideDay, bootstrap.rooms).filter((roomId) => {
	...
	for (const roomId of used) {
		...
		const { primary, primaryIsLive, following } = findPrimaryAndFollowing(list, guideMinutes);
		...
		if (primary) renderEventMini(primaryBlock, primary, room, bootstrap.tags);
			...
			p.textContent = list.length === 0 ? "Nothing scheduled in this room." : "All done here for today.";
...
export function initScheduleUi(bootstrap: ClientBootstrap) {
	...
	requestAnimationFrame(() => {
		requestAnimationFrame(() => scrollScheduleCarouselToTodayIfNeeded(bootstrap));
	});
}
